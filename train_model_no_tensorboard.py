    ...
    for phone,phone_lengths,pitch,pitchf,spec,spec_lengths,wave,sid in data_iterator:
        ...
        with autocast(enabled=True):
            ...
            with autocast(enabled=False):
                ...
                loss_gen_all = loss_gen + feature_loss(fmap_r, fmap_g) + (F.l1_loss(y_mel, y_hat_mel) * configs.train.c_mel) + (kl_loss(z_p, logs_q, m_p, logs_p, z_mask) * configs.train.c_kl)
        optim_g.zero_grad()
